    ...
    def first_level_frame(self,arcs,node):  #SLR second_level_relation
        ...
        for relation_type in node.children:
            ...
            for child_idx in node.children[relation_type]:
                frame[relation_type] = {'word':arcs[child_idx].word,'node':arcs[child_idx].idx,'SLR':self.second_level_frame(arcs,arcs[child_idx])}
        return frame
            ...
            for relation_type in node.children:
                ...
                for child_idx in node.children[relation_type]:
                    if child_idx in node2frame_dict:
                        ...
                        frame[relation_type].append({'word':arcs[child_idx].word,'node':arcs[child_idx].idx,'SLR':self.second_level_frame(arcs,arcs[child_idx])})
